	...
	if (!ev.metaKey) {
		...
		if ((ev.which >= 37 && ev.which <= 40) || (ev.which == 33 || ev.which == 34))
		{
			...
			else if (ev.which == 39)
			{
				...
				if (active.data('result').hasContext)
				{
					contexts.push({
						'term': $('#query').val()
					});
					$('#query').val('');
					ipc.send('database:context', active.data('result')._id);
				}
			...
			else if (ev.which == 37)
			{
				if (contexts.length > 0)
				{
					...
					$('#query').val(context.term);
					ipc.send('search', $('#query').val());
				}
				return false;
			}
			// page down
			else if (ev.which ==  34 || ev.which == 33)
			{
				var step = 7;
...
$('body').on('keyup', function(ev) {
	...
	else
	{
		...
		{
			var term = $('#query').val();
			render_results(_.filter(contextresults, function(result) {
				console.log('RESULT = ' + result.label + ' TERM = ' + term);
				return result.label.toUpperCase().indexOf(term.toUpperCase()) != -1;
			}));
			console.log("CTXT TERM = " + term);
		}
		return true;
	}
});
...
ipc.on('results', function(results) {
	...
	render_results.apply(null, arguments);
});
